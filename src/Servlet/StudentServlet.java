...
public class StudentServlet extends HttpServlet {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 8740754045571410896L;
	...
	private void addStudent(HttpServletRequest request, HttpServletResponse response) {
		...
		student.setQq(xibu);
		student.setSex(sex);
		student.setSn(SnUtil.generateSn(clazzId));
		StudentDao studentDao = new StudentDao();
		if(studentDao.addStuent(student)){
			try {
				response.getWriter().write("success");
			} catch (IOException e) {
				// TODO Auto-generated catch block
	...
	private void studentList(HttpServletRequest request, HttpServletResponse response) throws IOException {
		// TODO Auto-generated method stub
		try {
			request.getRequestDispatcher("view/stuentList.jsp").forward(request, response);
